...
static struct ScreenCell screen[SCREEN_HEIGHT][SCREEN_WIDTH];
static SDL_Vertex colorVerts[SCREEN_WIDTH * SCREEN_HEIGHT * 6];
static SDL_Vertex glyphVerts[SCREEN_WIDTH * SCREEN_HEIGHT * 6];

// static Uint64 last_frame;

static const char font_bmp[]; // Tentative, see bottom of file
static const size_t font_bmp_size;

static const SDL_Color palette[16]; // Tentative, see bottom of file
...
int init_screen() {
  ...
  // Load the font
  {
    SDL_Surface *s = SDL_LoadBMP_RW(SDL_RWFromConstMem(font_bmp, (int)font_bmp_size), 1);
    if (!s)
      return 1;
...

  {
    ...
    const char *oldQuality = SDL_GetHint(SDL_HINT_RENDER_SCALE_QUALITY);
...
//==================================================get_screen====================================================
struct ScreenCell get_screen(int x, int y) { return screen[y][x]; }

//==================================================set_screen====================================================
void set_screen(int x, int y, struct ScreenCell cell) {
  screen[y][x] = cell;

  int idx = (y * SCREEN_WIDTH + x) * 6;
  for (int i = 0; i < 6; i++) {
    colorVerts[idx + i].color = palette[cell.bg];
    glyphVerts[idx + i].color = palette[cell.fg];
  }

  const float gx = (float)(cell.glyph % 16 * FONT_WIDTH) / (FONT_WIDTH * 16);
  const float gy = (float)(cell.glyph / 16 * FONT_HEIGHT) / (FONT_HEIGHT * 16);
...
//=================================================clear_screen===================================================
void clear_screen() { clear_screen_to((struct ScreenCell){.fg = WHITE, .bg = BLACK, .glyph = ' '}); }

void clear_screen_to(struct ScreenCell clearCell) {
  for (int y = 0; y < SCREEN_HEIGHT; y++)
    for (int x = 0; x < SCREEN_WIDTH; x++)
      set_screen(x, y, clearCell);
}
...
static const char font_bmp[] = "\x42\x4d\x3e\x12\x00\x00\x00\x00\x00\x00\x3e\x00\x00\x00\x28\x00" //
